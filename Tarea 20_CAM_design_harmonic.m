...
theta1 = theta(1+B0:1+B1);       % dwell interval (bottom)
theta2 = theta(1+B1:1+B2);       % rise interval
theta3 = theta(1+B2:1+B3);       % return interval
theta4 = theta(1+B3:1+B4);       % dwell interval (bottom)

delta1 = (B1-B0)*pi/180;         % angle of rotation for rise
...
x2 = x2(2:end-1);
%% return 
x3 = (L/(2*pi))*(  ( -(2*pi*(theta3-beta(2)))/delta3) + sin(  (2*pi*(theta3-beta(2)))/delta3 ))+L;
%% dwell
x4 = zeros(1,length(theta4));
x4 = x4(2:end);
%% Follower displacement and cam profile
x   = [x1,x2,x3,x4];                     % follower's displacement (m)
